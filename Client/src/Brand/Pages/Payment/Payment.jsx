...
const BrandPayment = () => {
  ...
  const loadPaymentDetails = async () => {
    try {
      ...
      const res = await axios.get(
        `http://127.0.0.1:8000/BrandPaymentDetails/${requestid}/`
      );
  ...
  const payNow = async () => {
    ...
    try {
      ...
      const res = await axios.post("http://127.0.0.1:8000/BrandPaymentComplete/", {
      ...
      if (res.data.status === "success") {
        ...
        navigate(`/brand/chat/${paymentData.room_id}`);
      } else {
    ...
    <div className={style.page}>
      <div className={style.paymentCard}>
        <div className={style.leftSection}>
          <p className={style.tag}>Brand Payment</p>
          <h2 className={style.title}>Complete Influencer Payment</h2>
          <p className={style.subtext}>
            Request ID: <span>{paymentData.request_id}</span>
          </p>

          <div className={style.methodBox}>
            <button
              className={`${style.methodBtn} ${paymentMethod === "card" ? style.activeMethod : ""}`}
            ...
            <button
              className={`${style.methodBtn} ${paymentMethod === "upi" ? style.activeMethod : ""}`}
            ...
            <div className={style.formSection}>
              ...
              <div className={style.inputGroup}>
                ...
                <input
                  ...
                  maxLength={19}
                />
                {errors.cardNumber && <p className={style.error}>{errors.cardNumber}</p>}
              </div>
              ...
              <div className={style.row}>
                <div className={style.inputGroup}>
                  ...
                  <input
                    ...
                    maxLength={5}
                  />
                  {errors.expiry && <p className={style.error}>{errors.expiry}</p>}
                </div>
          ...
          <button
            ...
            onClick={payNow}
            disabled={loading || String(paymentData.payment_status) === "1"}
            type="button"
          >
            {String(paymentData.payment_status) === "1"
              ? "Already Paid"
              : loading
              ? "Processing..."
        ...
        <div className={style.rightSection}>
          <div className={style.summaryCard}>
            ...
            <div className={style.summaryRow}>
              <span>Brand</span>
              <span>{paymentData.brand_name}</span>
            </div>

            <div className={style.summaryRow}>
              <span>Influencer</span>
              <span>{paymentData.influencer_name}</span>
            </div>

            <div className={style.summaryRow}>
              <span>Product</span>
              <span>{paymentData.product_name}</span>
            </div>
            ...
            <div className={style.summaryRow}>
              ...
              <span>{String(paymentData.status) === "1" ? "Accepted" : "Pending"}</span>
            </div>

            <div className={style.summaryRow}>
              <span>Payment Status</span>
              <span className={style.pending}>
                {String(paymentData.payment_status) === "1" ? "Paid" : "Pending"}
              </span>
            </div>
          ...
          <div className={style.secureBox}>
            <h4>Safe Payment</h4>
            <p>This brand payment page is used after influencer offer acceptance.</p>
          </div>
        </div>
      </div>
    </div>
